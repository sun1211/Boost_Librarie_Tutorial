...
struct do_modifications {
    //check that the passed vectors have the same size
    ...
    //modifying metafunction
    //Compares the second parameter passed to it (via _2) with an unsigned type.
    // If types are equal, makes the first parameter passed to it (via _1) unsigned and returns that type.
    // Otherwise, it compares the second parameter passed to it (via _2) with a constant type.
    // If types are equal, it makes the first parameter passed to it (via _1) constant and, returns that type.
    // Otherwise, it returns the first parameter passed to it (via _1).
