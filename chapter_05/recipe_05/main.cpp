...
//our online service is somehow slow and the profilers show that the problem is in the users_online class.
//Any operation acquires a unique lock on the mutex_ variable,
//so even getting resources results in waiting on a locked mutex.
//As some of the resources are hard to copy, the critical sections consume a lot of time,
//slowing down any operation on the users_online class.
class users_online {
...
class users_online {
    //Replace boost::mutex with boost::shared_mutex.
